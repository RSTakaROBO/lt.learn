 ...
 */
export const WizardProgressDots = forwardRef(function WizardProgressDots({ step, total = 3 }, ref) {
    ...
    return (
        <div
            ...
            role="progressbar"
            aria-label={STR.wizard.stagesAria}
            aria-valuenow={n}
            aria-valuemin={1}
            aria-valuemax={total}
        >
            {Array.from({ length: total }, (_, i) => (
                <span
                    key={i}
                    className={["wizard-dot", i + 1 === n ? "is-active" : ""]
                        .filter(Boolean)
                        .join(" ")}
                    aria-hidden="true"
                />
