  ...
  await Promise.all(
    [...uniqueFiles].map(async (file) => {
      try {
        ...
        if (!res.ok) throw new Error(fmt(STR.errors.fileBadStatus, { ref: file, status: res.status }));
...
export function renderPackList(packs) {
  ...
  for (const p of packs) {
    ...
    const title = packDisplayTitle(p);

    const wrap = createCheckboxTileLabel({
      id: inputId,
      value: p.id,
      checked: Array.isArray(saved) && saved.includes(p.id),
      titleText: title,
    ...
    if (p.custom) {
      ...
      del.setAttribute("aria-label", fmt(STR.errors.deletePackAria, { title }));
...
function fillPackWordCountsFromFileMap(packs, fileMap) {
  ...
  for (const p of packs) {
    ...
    countEl.textContent = ok ? formatWordCountRu(sum) : STR.errors.countFailed;
  }
}
...
export async function loadManifestAndRenderPacks() {
  ...
  if (!manifestRes.ok) throw new Error(fmt(STR.errors.fileBadStatus, { ref: "manifest.json", status: manifestRes.status }));
  ...
  if (!els.packList.querySelector("input")) {
    throw new Error(STR.errors.manifestNoValidPack);
  }
